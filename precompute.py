...
class DataAnalysisPrecomputer:
    ...
    def _compute_dashboard_data(self) -> Dict:
        ...
        if self.t41_df is not None and len(self.t41_df) > 0 and self.sward_config is not None:
            ...
            # 🆕 개선된 알고리즘: 2분 단위 ≥3회 = 활성
            t41_stats = t41_with_loc.copy()
            
            # 10초 단위 반올림 샘플링 (중복 제거)
            t41_stats['timestamp_10s'] = ((t41_stats['time'].astype('int64') // 10**9 + 5) // 10) * 10
            t41_stats = t41_stats.drop_duplicates(subset=['mac', 'timestamp_10s', 'building', 'level'])
            
            # 2분 단위 bin 생성
            t41_stats['two_min_bin'] = t41_stats['time'].dt.floor('2min')
            t41_stats['bin_index'] = t41_stats['time'].dt.hour * 6 + t41_stats['time'].dt.minute // 10
            
            # 2분 단위 신호 수 계산
            two_min_signal = t41_stats.groupby(['mac', 'two_min_bin', 'building', 'level']).size().reset_index(name='signals')
            two_min_signal['is_active'] = two_min_signal['signals'] >= 3  # 2분에 3회 이상 = Active
            two_min_signal['bin_index'] = (
                two_min_signal['two_min_bin'].dt.hour * 6 + 
                two_min_signal['two_min_bin'].dt.minute // 10
            )
            
            # 10분 bin당 활성 여부 (5개 2분 bins 중 1개 이상 활성이면 Active)
            mac_bin_activity = two_min_signal.groupby(['mac', 'bin_index', 'building', 'level']).agg({
                'is_active': 'any'
            }).reset_index()
            ...
            if self.flow_df is not None and len(self.flow_df) > 0:
                ...
                def calc_tvm_for_filter(t41_data, flow_data, filter_name):
                    """T-Ward vs Mobile 10분 단위 비교 데이터 계산 (활성 T41만)"""
                    # T41: 활성 worker만 카운트 (mac_bin_activity에서 is_active=True인 것만)
                    # mac_bin_activity 데이터를 활용
                    t41_active = mac_bin_activity[mac_bin_activity['is_active']].copy()
                    
                    # Building/Level 필터 적용
                    if 'building' in t41_data.columns:
                        buildings_in_data = t41_data['building'].dropna().unique()
                        if len(buildings_in_data) > 0:
                            t41_active = t41_active[t41_active['building'].isin(buildings_in_data)]
                    if 'level' in t41_data.columns:
                        levels_in_data = t41_data['level'].dropna().unique()
                        if len(levels_in_data) > 0:
                            t41_active = t41_active[t41_active['level'].isin(levels_in_data)]
                    
                    # 10분 bin별 활성 unique mac 카운트
                    t41_counts = t41_active.groupby('bin_index')['mac'].nunique().reset_index()
                    t41_counts.columns = ['bin_index', 't41_count']
                    
                    # Flow: 1분 unique MAC → 10분 평균 (Mobile 더 정밀하게)
                    flow_data = flow_data.copy()
                    flow_data['one_min_bin'] = (flow_data['time'].dt.hour * 60 + flow_data['time'].dt.minute)
                    flow_data['ten_min_bin'] = (flow_data['time'].dt.hour * 6 + flow_data['time'].dt.minute // 10)
                    
                    one_min_counts = flow_data.groupby('one_min_bin')['mac'].nunique().reset_index()
                    one_min_counts.columns = ['one_min_bin', 'device_count']
                    one_min_counts['ten_min_bin'] = one_min_counts['one_min_bin'] // 10
                    
                    flow_ten_min = one_min_counts.groupby('ten_min_bin')['device_count'].mean().reset_index()
